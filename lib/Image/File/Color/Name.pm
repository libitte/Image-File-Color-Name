...
sub fetch {
	...
	Carp::croak(q{$filename is required})
		unless defined $filename;

	my $hsvs_ref = $self->_extract_hsv($filename);
	my $color_names_ref = $self->_allot_color_name($hsvs_ref);
	my $color_palette = $self->_count_up_for_each_color_name($color_names_ref);
	my $color_name = $self->_main_color_name($color_palette);
...
libitte E<lt>nhachi8@gmail.comE<gt>
